...
(define (cst-func suffixes)
  (cond
    ([or (null? suffixes) (null? (cdr suffixes))] suffixes)
    (else (let* ((prefix (longest-common-prefix-of-list suffixes)))
            (cons prefix (map (λ (word) (caddr (longest-common-prefix prefix word))) suffixes))))))
...
(define (suffixes->st labeling-func suffixes alphabet)
  (let* ((partial-st-with-null (map (λ (chr) (labeling-func (get-ch-words suffixes chr))) alphabet))
         (partial-st (filter (λ (x) (not (null? x))) partial-st-with-null)))
    (map (λ (branch) (if (null? (cdr branch))
                         branch
...
(define (text->st labeling-func)
  (λ (text)
    (let* ((suffixes (get-suffixes (append text '(#\$))))
           (alphabet (sort (remove-duplicates (map car suffixes)) char<?)))
